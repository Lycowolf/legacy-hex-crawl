...
func _done():
	hero_is_finished(' has died of ' + death_cause.to_lower())
...
func _become_yetti():
	trigger_message('Screw it, you never wanted to be a civilized human anyway. You heed the call of the woods and became a wild man.')
	hero_is_finished(' vanished into woods and became a Yetti')

func _give_up():
	trigger_message('You kinda just lie down and stay where you are. Its not bad place, but it kinda limits heroic opportunities.')
	hero_is_finished(' got lost and gave up.')
	
func _wander_more():
	var how_long = randi_range(1,6)
	trigger_message('You wandered {0} years, and finally found your bearings.'.format([how_long]) + \
	'\n\nYou became an experienced vagabond.' +\
	'\n\nNOTE: This makes it harder to get lost.', 'Nice!')
	...
	%StatPanel.conditions['Lost'].limit += 5
